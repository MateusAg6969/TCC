...
| **perfil**                 | Sub‑documento                     | Dados pessoais, identidade e acadêmicos.      | `nome`: 3‑100 caract., trim.<br>`email`: único, regex, lowercase.<br>`url_personalizada`: único, regex.<br>`bio`: ≤ 500 caract.<br>`status_vinculo`: enum (`estudante`, `egresso`, `servidor`).<br>`privacidade`: enum (`publico`, `privado`).<br>`verificado`: Boolean.<br>`curso`: String (opcional).<br>`ano`: String (opcional, ex-aluno para egresso). | `perfil.email` (único)<br>`perfil.url_personalizada` (único) |
...
| **/auth/login**                      | `POST`             | [Não]             | Autentica usuário com `email`, `senha` e `rememberMe` (opcional). Gera **JWT** (`accessToken`) e **Refresh** (`refreshToken` por 30 dias se `rememberMe` for true, ou por 1 dia/sessão se false). | `bcrypt`                                           |
